...
| **version**|This field is used to store the version of the integration running in the user's integrator.io account.  Whenever you push any managed updates to a user's integrator.io account you should always increment this version field as the final step.|
| **updateInProgress**|This field is automatically set to 'true' whenever a managed update is pushed to a user's integrator.io account.  You must explicitly set this field back to 'false' when your update is done.  While updateInProgress is 'true' the user will not be allowed to change any settings or mappings in their integration (this protection was put in place to prevent race conditions corrupting an integration).|
| **install**|???|

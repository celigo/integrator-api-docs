...
| **version**|This field is used to store the version of the integration running in the user's integrator.io account.  Whenever you push any managed updates to the integrations running in a user's integrator.io account you should always increment the version field as the final step in your backend connector code (i.e. so that you can then see the update was successful from your master integrator.io account).|
| **updateInProgress**|This field is automatically set to 'true' whenever a managed update is pushed to the integration running in an end user's integrator.io account.  Your backend connector code (that performs the update) must set this field back to 'false' when the update is done.  While updateInProgress is 'true' the end user will not be allowed to submit changes to the integration (to avoid data corruption and/or race conditions), and they will receive an error in the UI if they attemp to do so, etc...|
| **install**|???|
